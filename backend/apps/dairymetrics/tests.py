...
class DairyMetricsV2DemoTests(TestCase):
    ...
    def test_metrics_v2_demo_renders_member_sections(self):
        ...
        self.assertContains(response, '<option value="month" selected>月</option>', html=True)
        self.assertContains(response, '<option value="period">路程</option>', html=True)
        self.assertContains(response, '<option value="range">期間指定</option>', html=True)
