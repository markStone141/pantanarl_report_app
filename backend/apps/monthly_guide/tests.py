...
class MonthlyGuideTests(TestCase):
    def test_monthly_guide_page_renders(self):
        ...
        self.assertContains(response, "フランス語")
        self.assertContains(response, "スペイン語")
        self.assertContains(response, "ポルトガル語")
        self.assertContains(response, "ロシア語")
        self.assertContains(response, "韓国語")
        self.assertContains(response, "インドネシア語")
        self.assertContains(response, "マレー語")
        ...
        self.assertContains(response, "ヒンディー語")
        self.assertContains(response, "シンハラ語")
        self.assertContains(response, "ベンガル語")
        self.assertContains(response, "中国語")
        self.assertContains(response, "<option", count=17)
