...
作業は小さな単位で実行し、各ループ後に進捗、変更量、未解決問題を確認する。上限以内なら継続し、上限に達した場合は無理に完了まで進めない。

以下のいずれかに該当した場合、作業を停止する。
...
停止時は `HANDOFF.md` を作成し、`PROJECT_STATE.md` の現在フェーズまたは状態を `paused` に更新する。停止後は追加変更を行わず、`project_manager` へ返す。

`HANDOFF.md` には必ず以下を記録する。
...
作業が `paused` になった場合、`project_manager` は中断理由を確認し、未完了作業を小さく分割する。必要なら別ロールへ再割り当てし、仕様判断が必要ならユーザー確認へ回す。再開条件を満たした場合だけ作業を再開する。

再分割例:

```text
元の作業: 認証機能全体をリファクタリングする
停止理由: 変更範囲が大きすぎる
再分割:
1. バリデーションの共通化
2. 例外処理の整理
...
これらに該当する場合、`project_manager` は完了扱いにする前に、構成監査の要否、監査結果、修正が必要な場合の優先度を確認する。
...
- `paused`: 長期化、範囲超過、仕様判断待ちなどにより、`HANDOFF.md` を作成して作業を中断した。
...
- 未決事項
- ブロッカー
- 状態が `paused` の場合は中断理由と `HANDOFF.md` の参照
- 依存関係
- 次に行う作業
