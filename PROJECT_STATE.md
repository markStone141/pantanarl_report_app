...
- 全アプリUI展開・工程1: Performance基準UIの確定
...
- `performance` の管理者ダッシュボードを全アプリ展開の基準UIとして確定する工程を開始した。
- 有効メンバー一覧を、直近3稼働、3稼働連続0件、3稼働連続実績ありを第一視線で判別できる構造へ再設計する。
- 全アプリUI展開の工程、追加要件、完了条件を `backend/docs/UI_ROLLOUT_PLAN.md` に固定した。
...
- 対象: `performance` 管理者ダッシュボードと有効メンバー一覧。
- 対象外: 全アプリ共通NAVの実装、他アプリの画面変更、認証変更。
- 有効メンバー一覧に直近3回の活動終了済み稼働を表示する。
- 3稼働連続0件、3稼働連続1件以上、その他を見分けられ、状態別件数と絞り込みを提供する。
- 未報告、活動中、活動終了済み0件を混同しない。
- PC／モバイル、UN／WV、補正込み実績、個人詳細導線、空状態を検証する。
...
- 工程1として、有効メンバーカードの直近3稼働表示用データと状態別集計のテストを先に追加する。
- テンプレートとCSSを、直近3稼働中心の一覧と状態別フィルターへ変更する。
- PC／モバイル表示と操作を確認し、工程1の検証・レビュー・コミットを完了する。
- その後、工程2の全アプリ・全画面・全メニュー台帳へ進む。
