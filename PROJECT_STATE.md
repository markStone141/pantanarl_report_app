...
- 工程3の試験実装と機械検証は完了。公開環境でPC／モバイルの視覚確認後に基準NAVとして確定する。
- 次工程4では、試験NAVを共通UI基盤の一部として他画面へ展開可能な構造へ整理する。
...
- 対象: 全アプリ共通のメニュー定義、PCサイドNAV、スマホ用グループ化ドロワー、簡潔なページヘッダー、`dashboard` と `performance` の試験適用。
- 対象外: 各アプリ本文UIの全面改修、認証方式変更、公開環境への反映。
- PCとスマホが同じ名称、順序、グループ、権限条件を使う。
- PCは左サイドNAV、スマホはドロワーを使い、現在位置を明確にする。
- フォーカス、キーボード操作、Escape、背景スクロール、ログアウトの分離を確認する。
- `dashboard` と `performance` の既存URL・権限・主要操作を維持する。
- 対象テストと公開前必須チェックを成功させ、工程単位でコミットする。

## 工程3の終了判定

- 実装: 完了。`dashboard` と `performance` の管理トップが共通メニューHTMLを利用する。
- 権限: 管理・設定／個別業務は管理者だけに表示する。一般報告者向けPerformance表示では非表示になる。
- 操作: 現在位置、Escape、フォーカス循環、背景スクロール停止、ログアウト分離を実装済み。
- 検証: 対象41テスト、全380テスト、Django check、migration、BOM、JavaScript構文、差分検査が成功。
...
- 工程4の開始時に、色、余白、ページヘッダー、セクション、カード、フォーム、状態表示の共通化範囲を固定する。
- 工程3の共通NAVを各画面へ展開できる部品として整理し、既存個別NAVとの置換順を決める。
- 別管理の公開作業として、監査済みブランチをGitHubへPUSH後、公開環境の主要導線を確認する。
