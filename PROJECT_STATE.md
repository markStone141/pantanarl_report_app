...
- 工程13「新UI全画面適用の再監査・完成」を進行中。工程13-3のDairyMetrics・Mail移行を完了し、次は工程13-4の概ね適用済み画面と独立UIの仕上げ。
...
- 共通NAVとDairyMetricsのモバイルハンバーガーを新 `ui-icon-button` へ移行し、独立UIに残る同操作は工程13-4で統一する。
...
- 次は工程13-4「概ね適用済み画面と独立UIの仕上げ」を進める。
...
- 工程13-4として、`targets`、`reports`、`talks`、`testimony`、`monthly_guide`、`mosaic`、`accounts`、エラー画面を仕上げ監査する。
- Mosaic等の独立UIに残るモバイルハンバーガーを共通 `ui-icon-button` へ統一する。
