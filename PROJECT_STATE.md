...
- アプリ単位の構成、責務、依存、テスト見直し。`performance` リファクタリング計画は8工程中4工程まで完了。
...
- `performance/views.py` はsnapshot構築、今日の明細、メンバーカードの分離後も、メンバー個別画面、補正実績、過去入力、AJAXが混在している。
...
- `performance` リファクタリング計画の工程5として、メンバー個別・過去実績contextを `member_pages.py` へ分離する。
