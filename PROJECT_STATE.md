...
- 工程4の共通UI基盤と試験適用は完了。公開環境でPC／モバイルの視覚確認後に見た目を最終調整する。
- 次工程5では `dashboard`、`targets`、`mail` の管理・設定画面へ、共通NAVとUI基盤を画面単位で展開する。
...
- 工程5の開始時に、`dashboard`、`targets`、`mail` の対象画面と置換順を固定する。
- 管理トップ、メンバー・部署、目標設定、メール設定・履歴へ共通NAVとUI基盤を順次適用する。
