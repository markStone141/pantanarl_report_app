...
- アプリ単位の構成、責務、依存、テスト見直し。次は `dashboard` のメンバー管理処理、または `mosaic` / `testimony` / `talks` を確認する。
...
- `dashboard/views.py` はまだ1000行超のため、メンバー管理・部署管理・メールテンプレート生成を別作業として追加分割する余地がある。
