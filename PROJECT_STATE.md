...
- グラフカードUI刷新を開始。工程0のモバイル条件指定とドロワーの重なり修正を完了し、次は工程1のグラフ台帳・共通カード基盤へ進む。工程分割は `backend/docs/GRAPH_CARD_UI_ROLLOUT_PLAN.md` を参照する。
- 全アプリUI展開・工程10完了。
...
- 工程14-2の横展開として、目標設定概要の現在月／現在路程を期間パネルと部署別目標カードへ変更した。メンバー管理一覧もカード化し、所属・メール・ログイン状態を可視化した。メンバー登録／編集は基本情報・所属部署・ログイン情報の3ブロックへ再編した。
...
## グラフカード刷新 工程2の終了判定（2026-08-14）

- 実装: 分析するページの達成率、月／路程推移、ランキング、属性分布、属性別平均金額を共通グラフカードへ移行した。
- 操作: 個人／全体タブ、ランキング指標、平均金額分類、既存Canvas IDとChart.js payloadを維持した。
- 状態表示: 月／路程、ランキング、属性分布、平均金額へカード内空状態を追加し、0件属性の空凡例を非表示にした。
- 視覚検証: 1440px／390pxでJavaScriptエラー、ページ横スクロール、カード操作部のはみ出しがないことを確認した。
- 自動検証: dairymetrics 44テスト、JavaScript構文、BOM、差分検査が成功した。
- 次工程: 振り返りレポートの分析グラフへ同じ構造を適用する。

## グラフカード刷新 工程3の終了判定（2026-08-14）

- 実装: 振り返りレポートの属性分布と属性別平均金額を、分析画面と同じ共通グラフカードへ移行した。
- 印刷: 分類セレクトだけを印刷対象外とし、カード・見出し・グラフ・空状態は印刷時も維持する。
- 状態表示: 0件の属性は空状態をカード内に表示し、値のない凡例は表示しない。
- 視覚検証: 1440px／390pxでJavaScriptエラー、ページ横スクロール、操作部のはみ出しがないことを確認した。
- 自動検証: レポート主要3テスト、Django check、差分検査が成功した。
- 次工程: 実績管理トップ、過去実績、メンバー詳細／履歴へ共通構造を展開する。

## グラフカード刷新 工程4の終了判定（2026-08-14）

- 実装: 実績管理トップ、過去実績、メンバー詳細、メンバー履歴の達成率・推移カードを共通グラフUIへ移行した。
- 操作: 推移件数の増減、金額／達成率／AP・CM／WV切替、Canvas ID、部署AJAX切替用クラスを維持した。
- モバイル: 推移モードを2列折返し、カード・操作・グラフ枠を390px内に収めた。
- 視覚検証: 4画面を1440px／390pxで確認し、HTTP 200、JavaScriptエラーなし、ページ横スクロールなしを確認した。
- 自動検証: performance 103テスト、JavaScript構文、Django check、差分検査が成功した。
- 次工程: 残存グラフ監査と全体統合検証を行う。

## グラフカード刷新 工程5の終了判定（2026-08-14）

- 監査: Canvas 19件が対象6テンプレートだけに存在し、各テンプレートが共通グラフカード契約を利用することを自動テスト化した。
- 整理: 入力画面で参照されていない旧ミニグラフCSSを削除した。
- 横断確認: 分析、振り返り、実績管理、過去実績、メンバー詳細／履歴を1440px／390pxで確認した。WV固有指標と補正実績は既存テストで回帰確認した。
- 検証: 全402テスト、Django check、migration差分、JavaScript構文、BOM、EOF／差分検査が成功した。
- 完了判定: 工程0〜5が完了。PUSH・公開は未実施。
