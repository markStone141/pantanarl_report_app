...
- 工程13「新UI全画面適用の再監査・完成」を開始。ボタンUIは適用済みだが、旧ヘッダー／旧NAVと共通シェル未構成画面が残るため、全画面完成工程として分離した。
...
- 工程13-0として、共通ドロワーのメニューリンクを明示遷移させ、メンバー管理が展開だけで止まる不具合を修正する。
- 工程13-1以降で、`dashboard`、`performance`、`dairymetrics`、`mail` の部分適用画面を共通シェルへ移行する。
...
- 工程13-0のメンバー管理メニュー遷移修正を検証・コミットする。
- 工程13-1として、メンバー管理系4画面の `app-shell`／`app-shell-content` を完成させる。
- 工程13-2以降は `UI_COMPLETION_ROLLOUT_PLAN.md` の順序で1工程ずつ実施する。
