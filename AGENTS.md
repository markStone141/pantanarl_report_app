...
大きな工程、複数ロールにまたがる作業、公開準備では、`project_manager` を工程の開始時と終了時に必ず通す。

- 開始時: 現在地、対象範囲、対象外、依存関係、担当観点、完了条件、公開判定基準を固定する
- 終了時: 検証結果、残リスク、修正要否、コミット/PUSH/公開状態、次工程を確認する
- 状態は会話履歴だけに残さず、`PROJECT_STATE.md` と作業ログへ反映する
- 進行中の主工程は原則1つに絞り、追加要望は同じ工程へ含めるか別工程へ送るかを明示する
